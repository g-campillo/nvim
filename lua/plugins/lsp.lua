return {
  ...
  config = function()
    ...
    -- JasperReports templates are XML; this is what gets lemminx to attach to them
    vim.filetype.add { extension = { jrxml = 'xml' } }

    -- Newest sdkman JDK matching a version glob, e.g. sdkman_jdk('2[1-9]*')
    local function sdkman_jdk(pat)
      local dirs = vim.fn.glob(vim.env.HOME .. '/.sdkman/candidates/java/' .. pat, false, true)
      table.sort(dirs)
      return dirs[#dirs]
    end
    ...
    local servers = {
      ...
      jdtls = {
        cmd_env = { JAVA_HOME = sdkman_jdk '2[1-9]*' },
        settings = {
          java = {
            configuration = {
              runtimes = { { name = 'JavaSE-17', path = sdkman_jdk '17*', default = true } },
            },
          },
        },
      },
    ...
    -- SourceKit-LSP (Swift) — ships with Xcode, not managed by Mason
